...
for dir in "$@" ; do
	...
	while read -r line; do
	    if [[ "$line" =~ $regex ]]; then
            # we excuse REPO_0 if print_rsync_iso has bsdtar (which is indication that REPO_0 comes from print_rsync_iso)
            echo "$known_destination_repos" | grep -q "${BASH_REMATCH[1]}\$" || ( [[ "$line" =~ REPO_0= ]] && grep -q bsdtar "$dir"/print_rsync_repo.sh ) || { >&2 echo "Destination REPO file wasnt found in print_rsync_repo output {${BASH_REMATCH[1]}}   {$known_destination_repos}"; : $((++errs)); continue 2; }
            checked=1
	    fi
